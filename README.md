...
Most commands interpret `.jl` files as Julia code and any other file extension as text. The keyword argument `parse_as=:julia` or `parse_as=:text` can be used to override this, but if the parsing type for a file changes then all existing handles to that file are invalidated.

Under the hood, the package keeps a cache of all files that it has already parsed into blocks.

## Getting block handles

`Handle(path, line, [pos=1])` - Returns a handle to the block containing the character at line `line`, character position `pos`. If that location is not inside a block, returns the next block after that location.
...
`handles(path)` / `handles(paths)` / `handles(root, glob)` - Returns a `Set` of `Handle`s to all blocks in a file, or in a set of files (including EOF blocks). If the keyword argument `includes` is `true`, then `include` statements are followed recursively.

The functions throw an `ArgumentError` if a Julia file cannot be parsed, or if a file contains invalid UTF-8.
...
`search(handles, trace)`/ `search(handles, exception)` - Returns handles to code referenced by the stack trace or an exception.
...
`CreateFile(path, new_code)` - An edit which creates a new file.

`MoveFile(old_path, new_path)` - An edit which renames or moves a file.
...
For safety, `apply!` refuses to apply an edit unless it has previously been displayed. REPL printing and calls to `Base.display(edit)` count, but calls to `string(edit)` do not. Handles automatically adapt to changing line numbers due to edits elsewhere in the file.
...
`docstring(handle)` - Returns the docstring as a string (not as Julia code), or `nothing` if the block has no docstring.
...
After files are modified outside CodeEdit.jl, existing handles may no longer match the file contents. The `reindex()` function attempts to update all handles to point to the correct block. This may invalidate some handles and modify the contents of others.
